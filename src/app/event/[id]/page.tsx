...
import { EventCoupons } from "./_components/EventCoupons";
  ...
  return (
    <div className="bg-[var(--cream)] min-h-screen">
      ...
      <div className="max-w-[960px] mx-auto px-6 pt-[80px] pb-16">
          ...
          <div className="mt-8 flex flex-col gap-7">
            {event.sections.map((section) => (
              <section key={section.heading}>
                <h2 className="text-[18px] text-black mb-2">{section.heading}</h2>
                <p className="text-[15px] text-[#3D3D3D] leading-[1.8]">{section.body}</p>
              </section>
            ))}
          </div>
        ) : (
        ...
        {/* 쿠폰 발급 — 진행 중인 이벤트만 */}
        {!isEnded && event.couponCodes && event.couponCodes.length > 0 && (
          <EventCoupons codes={event.couponCodes} />
        )}

        {/* CTA */}
        {!isEnded && (
          <div className="mt-12 flex flex-col sm:flex-row gap-3">
            <Link href="/store" className="btn btn-dark btn-md flex-1" style={{ justifyContent: "center" }}>
              스토어 둘러보기
            </Link>
            <Link
              href="/event"
              className="btn btn-ghost btn-md flex-1"
              style={{ justifyContent: "center", border: "1px solid var(--ink)" }}
            >
              다른 이벤트 보기
            </Link>
          </div>
